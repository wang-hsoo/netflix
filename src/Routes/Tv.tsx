...
function Home(){
    ...
    const bigTvMath = useRouteMatch<{tvId:string}>("/tv/:tvId");//현재위치를 확인함
    // const {data, isLoading} = useQuery<IGetMoviesResult>(["movies", "nowPlaying"], getMovies);
    const {data, isLoading} = useQuery<IGetTvResult>(["Tv", "tvPopular"], getTv);
   console.log(data);
    ...
    const incraseIndex = () => {
        ...
        if(data){
            ...
            setPrevCheck(false);
            if(prevCheck === false)
            {const totalTv = data?.results.length - 1; //이미 영화 하나는 사용하고 있기 때문에 -1을 해준다
            setLeaving(true);
            const maxIndex = Math.floor(totalTv / offset) - 1; //페이지가 0에서 시작하기때문에 -1을 해줘서 총 페이지 값을 맞쳐준다
    ...
    const onOverlayCick = () => history.push("/tv");
    const clickTv = 
        bigTvMath?.params.tvId && 
        data?.results.find(tv => tv.id === +bigTvMath.params.tvId)
                   ...
                   {bigTvMath ? ( 
                    <>
                        ...
                        <BigMovie 
                            ...
                            layoutId={bigTvMath.params.tvId}
                        >
                            {clickTv && (<>
                                <BigCover 
                                    style={{ 
                                        backgroundImage: `linear-gradient(to top, black, transparent), url(${makeImagePath(clickTv.backdrop_path, "w500")})`}} 
                                />
